...
const Assinatura = () => {
  ...
  const[notificationOkay,setNotificationOkay] = useState("off");
  ...
  function sendEmail (e){
    e.preventDefault();
    if(email.includes("@")){
    emailjs.sendForm('service_sdktioa', 'template_ke45mxk', form.current, 'yZIizqFPuunaW9Mpb')
      .then((result) => {
                
      setNotificationOkay("on")
      setTimeout(()=>{setNotificationOkay("off")}, 7000);   
  ...
  @import url('https://fonts.googleapis.com/css2?family=Montserrat&display=swap');
      ...
      <NotificationError state={notificationError}></NotificationError>
        <NotificationOkay state={notificationOkay}></NotificationOkay>
      ...
      <div className="left">
        ...
        <form ref={form} onSubmit={sendEmail} className="input">
            <div className="inputContainer">
                ...
                <input onChange={handleChange} name="to_email" placeholder="Insira seu email" type="text" />
                <button type="submit" value="Send">Assinar newsletter</button>
            </div>
        </form>
      </div>
      <div className="right">
